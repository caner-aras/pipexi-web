...
export async function PUT(
  ...
  try {
    const shiftBreak = await updateShiftBreak(breakId, {
    ...
    return NextResponse.json({ data: shiftBreak });
  } catch (error) {
    ...
    return NextResponse.json(
      { message: "Failed to update shift break." },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { message: "Failed to delete shift break." },
      { status: 500 }
    );
  }
}
