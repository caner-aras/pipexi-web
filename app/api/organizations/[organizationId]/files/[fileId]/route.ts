...
import { updateOrganizationFile } from "@/lib/server/services/organization-file.service";
...
export async function PUT(
  ...
  if (!organizationId || !fileId) {
    return NextResponse.json(
      { message: "Organization id and file id are required" },
      { status: 400 }
    );
  }
  ...
  try {
    const file = await updateOrganizationFile(
      organizationId,
      fileId,
      parsed.data
    );
